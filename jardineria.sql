...
select nombre,apellido1,apellido2,puesto from empleado where not puesto like 'Representante Ventas';
...
*/
select distinct(p.codigo_cliente),p.fecha_pago  from pago p
...
where p.estado like 'Pendiente' and p.fecha_entrega is null;
...
where p.estado='Rechazado' and extract(year from fecha_pedido)='2009';
/*12.Devuelve un listado de todos los pedidos que han sido entregados en el
mes de enero de cualquier año.*/
select * from pedido pedido
where estado='Entregado' and extract(month from fecha_pedido)='01';
...
select * from pago p
where extract(year from p.fecha_pago)='2008' and p.forma_pago like 'Paypal';
...
where p.gama like 'Ornamentales' and p.cantidad_en_stock>=100
...
where c.ciudad like 'Madrid' and c.codigo_empleado_rep_ventas in(11,30);

/*Consultas multitabla (Composición interna*/

/*1. Obtén un listado con el nombre de cada cliente y el nombre y apellido
de su representante de ventas.*/
select c.nombre_cliente,concat(e.nombre,' ',e.apellido1,' ',e.apellido2) Representante from cliente c
inner join empleado e
on c.codigo_empleado_rep_ventas=e.codigo_empleado;
/*2. Muestra el nombre de los clientes que hayan realizado pagos junto con
el nombre de sus representantes de ventas.*/

select c.nombre_cliente,c.codigo_cliente,p.codigo_cliente,c.codigo_empleado_rep_ventas,e.codigo_empleado from cliente c
inner join empleado e
on c.codigo_empleado_rep_ventas=e.codigo_empleado
inner join pago p
...
select p.codigo_cliente,c.nombre_cliente,e.nombre,o.ciudad from cliente c
inner join empleado e
on c.codigo_empleado_rep_ventas=e.codigo_empleado
inner join pago p
on c.codigo_cliente=p.codigo_cliente
...
where p.codigo_cliente is null;
;
SELECT pago.codigo_cliente,cliente.nombre_cliente, empleado.nombre, oficina.ciudad FROM cliente 
LEFT JOIN empleado 
...
select o.linea_direccion1,o.linea_direccion2,c.ciudad from oficina o
inner join empleado e
...
SELECT c.nombre, c.apellido1, 
	o.nombre AS nombre_jefe, o.apellido1 AS apellido_jefe 
    FROM empleado c join empleado o 
    ON(c.codigo_jefe = o.codigo_empleado);
    
/*9. Devuelve un listado que muestre el nombre de cada empleados, el
nombre de su jefe y el nombre del jefe de sus jefe.*/

select e.nombre jefe, j.nombre sub_jefe, sj.nombre empleado from empleado e
inner join empleado j
on e.codigo_empleado=j.codigo_jefe
inner join empleado sj
on j.codigo_empleado=sj.codigo_jefe
;
...
select distinct(g.gama) from gama_producto g
inner join producto p
on g.gama=p.gama
inner join detalle_pedido d
on p.codigo_producto=d.codigo_producto
inner join pedido o
on d.codigo_pedido=o.codigo_pedido
;

select distinct(p.gama),p.codigo_producto,count(o.codigo_cliente) from producto p
inner join detalle_pedido d
on p.codigo_producto=d.codigo_producto
inner join pedido o
on d.codigo_pedido=o.codigo_pedido
...
select c.*,p.*,d.* from cliente c
left join pago p
on c.codigo_cliente=p.codigo_cliente
left join pedido d
on c.codigo_cliente = d.codigo_cliente
where p.codigo_cliente is null and d.codigo_cliente is null;
...
select e.*,c.codigo_empleado_rep_ventas from empleado e
left join cliente c
on e.codigo_empleado=c.codigo_empleado_rep_ventas
where c.codigo_cliente is null;

/*6. Devuelve un listado que muestre solamente los empleados que no tienen un cliente asociado junto con los datos de la oficina donde trabajan.*/
select e.*,c.*,o.* from empleado e
left join cliente c
on e.codigo_empleado=c.codigo_empleado_rep_ventas
left join oficina o
on e.codigo_oficina=o.codigo_oficina
...
left join pedido o
on d.codigo_pedido=o.codigo_pedido
where o.codigo_pedido is null;

/*9. Devuelve un listado de los productos que nunca han aparecido en un pedido. El resultado debe mostrar el nombre, la descripción y la imagen del producto.*/

select p.nombre,p.descripcion,g.imagen  from producto p
left join detalle_pedido d
on p.codigo_producto=d.codigo_producto
left join pedido o
on d.codigo_pedido=o.codigo_pedido
...
left join producto t
on t.codigo_producto=d.codigo_producto
where t.gama like 'Frutales' and c.codigo_empleado_rep_ventas is null
...
select gama,codigo_producto from producto;

select o.codigo_oficina,e.codigo_empleado,c.codigo_cliente,p.codigo_cliente,d.codigo_pedido,t.codigo_producto,t.gama from oficina o
left join empleado e
on e.codigo_oficina=o.codigo_oficina
left join cliente c
on c.codigo_empleado_rep_ventas=e.codigo_empleado
...
join empleado em 
on (o.codigo_oficina=em.codigo_oficina)
join cliente cl 
on (cl.codigo_empleado_rep_ventas=em.codigo_empleado)
join pedido pd 
on (cl.codigo_cliente=pd.codigo_cliente)
join detalle_pedido dt_pd 
on(dt_pd.codigo_pedido=pd.codigo_pedido)
join producto pr 
on (dt_pd.codigo_producto=pr.codigo_producto)
join gama_producto g_pr 
on (pr.gama=g_pr.gama)
where g_pr.gama like 'Frutales' and (cl.codigo_empleado_rep_ventas <> all(select codigo_empleado from empleado e join oficina o on (o.codigo_oficina=e.codigo_oficina)));
/*11. Devuelve un listado con los clientes que han realizado algún pedido pero no han realizado ningún pago.*/
select c.*,p.*,a.* from cliente c
left join pedido p
on p.codigo_cliente=p.codigo_cliente
left join pago a
...
Join pago a on ( a.codigo_cliente= c.codigo_cliente) 
Where (a.codigo_cliente <> c.codigo_cliente);
/*12.Devuelve un listado con los datos de los empleados que no tienen clientes asociados y el nombre de su jefe asociado.*/
select e.nombre empleado,j.* from empleado e
left join cliente c
on c.codigo_empleado_rep_ventas=e.codigo_empleado
left join empleado j
on j.codigo_jefe=e.codigo_empleado
where c.codigo_empleado_rep_ventas is null;
/**/
...
select p.estado, count(estado) from pedido p
group by p.estado
...
select count(p.id_transaccion),p.fecha_pago, c.nombre_contacto,c.apellido_contacto from cliente c
inner join pago p
on p.codigo_cliente=c.codigo_cliente
group by c.nombre_contacto;

select nombre_cliente, min(fecha_pago) as Primer_Pedido, max(fecha_pago) as 
Último_Pedido from cliente
join pago 
group by nombre_cliente;

/*12.Calcula el número de productos diferentes que hay en cada uno de los pedidos.*/
select o.codigo_pedido,count(p.codigo_producto) from producto p
inner join detalle_pedido o
on o.codigo_producto=p.codigo_producto
group by o.codigo_pedido;
...
select d.codigo_pedido,sum(d.cantidad) total from detalle_pedido d
...
select d.codigo_producto,sum(d.cantidad) total from detalle_pedido d
group by d.codigo_producto order by total desc limit 20;
...
sum(((precio_unidad*cantidad)*0.21)+(precio_unidad*cantidad)) as Total_Facturado 
from detalle_pedido;
...
sum(((precio_unidad*cantidad)*0.21)+(precio_unidad*cantidad)) as 
Total_Facturado_IVA from detalle_pedido
...
select p.fecha_pago,sum(p.total) from pago p
...
SELECT p.nombre, d.cantidad FROM producto p
INNER JOIN detalle_pedido d 
ON p.codigo_producto=d.codigo_producto 
WHERE d.cantidad =(SELECT MAX(cantidad) from detalle_pedido);


/*4. Los clientes cuyo límite de crédito sea mayor que los pagos que haya realizado. (Sin utilizar INNER JOIN).*/

select c.* from cliente c
where c.limite_credito>(select max(total) from pago where c.limite_credito>total);

select cl.nombre_cliente , cl.limite_credito , p.total from cliente cl, pago p 
...
select e.nombre,e.apellido1,e.apellido2,email from empleado e
where e.codigo_jefe=(select codigo_empleado from empleado where nombre like 'Alberto' and apellido1 like 'Soria');

/**/
/*2 Subconsultas con ALL y ANY*/
/**/

/*8. Devuelve el nombre del cliente con mayor límite de crédito.*/
select c.nombre_cliente,c.limite_credito from cliente c
where  c.limite_credito = any(select max(limite_credito) from cliente);
...
where p.cantidad_en_stock = any(select min(cantidad_en_stock) from producto);
...
where e.codigo_empleado not in(select codigo_empleado_rep_ventas from cliente where e.codigo_empleado = codigo_empleado_rep_ventas);

SELECT nombre, apellido1 FROM empleado 
WHERE codigo_empleado NOT IN (SELECT codigo_empleado_rep_ventas FROM 
cliente WHERE codigo_empleado_rep_ventas=codigo_empleado);

/*12.Devuelve un listado que muestre solamente los clientes que no han realizado ningún pago.*/
select c.* from cliente c
where c.codigo_cliente not in(select codigo_cliente from pago where codigo_cliente=c.codigo_cliente);
/*13.Devuelve un listado que muestre solamente los clientes que sí han realizado algún pago.*/
select c.* from cliente c
where c.codigo_cliente in(select codigo_cliente from pago where codigo_cliente=c.codigo_cliente);

/*14.Devuelve un listado de los productos que nunca han aparecido en un pedido.*/

select p.* from producto p
where p.codigo_producto not in(select codigo_producto from detalle_pedido where p.codigo_producto=codigo_producto);
...
cliente.*/
select nombre,apellido1,apellido2,puesto,telefono from empleado e
inner join oficina o
on e.codigo_oficina=o.codigo_oficina
where codigo_empleado not in(select codigo_empleado_rep_ventas from cliente where codigo_empleado=codigo_empleado_rep_ventas);
...
inner join empleado e
on e.codigo_oficina=o.codigo_oficina
where e.codigo_empleado not in(select codigo_empleado_rep_ventas from cliente where codigo_empleado=codigo_empleado_rep_ventas);

/*17.Devuelve un listado con los clientes que han realizado algún pedido
pero no han realizado ningún pago.*/
SELECT nombre_cliente AS nombre , estado from cliente INNER JOIN pedido ON 
cliente.codigo_cliente=pedido.codigo_cliente INNER JOIN pago ON 
pedido.codigo_cliente=pago.codigo_cliente WHERE cliente.codigo_cliente IN 
(SELECT codigo_cliente FROM pago WHERE codigo_cliente=pedido.codigo_cliente);
/**/
...
left join pago p
on c.codigo_cliente=p.codigo_cliente
where not exists(select codigo_cliente from pago where c.codigo_cliente=p.codigo_cliente);
/*19.Devuelve un listado que muestre solamente los clientes que sí han realizado algún pago.*/
select c.nombre_cliente from cliente c
left join pago p
on c.codigo_cliente=p.codigo_cliente
where exists(select codigo_cliente from pago where c.codigo_cliente=p.codigo_cliente);
/*20.Devuelve un listado de los productos que nunca han aparecido en un pedido.*/

select p.* from producto p
left join detalle_pedido d
on p.codigo_producto=d.codigo_producto
where not exists(select nombre from producto where p.codigo_producto=d.codigo_producto);

SELECT nombre AS Nombre_Producto FROM producto AS p LEFT JOIN
detalle_pedido AS dp ON p.codigo_producto=dp.codigo_producto WHERE NOT
EXISTS (SELECT nombre FROM producto WHERE
p.codigo_producto=dp.codigo_producto);
/*21.Devuelve un listado de los productos que han aparecido en un pedido alguna vez.*/
select p.* from producto p
left join detalle_pedido d
on p.codigo_producto=d.codigo_producto
where exists(select nombre from producto where p.codigo_producto=d.codigo_producto);

SELECT nombre AS Nombre_Producto FROM producto AS p LEFT JOIN
detalle_pedido AS dp ON p.codigo_producto=dp.codigo_producto WHERE EXISTS
(SELECT nombre FROM producto WHERE p.codigo_producto=dp.codigo_producto);
...
sELECT c.nombre_cliente , COUNT(p.codigo_cliente)AS Pedidos
FROM cliente c, pedido p WHERE c.codigo_cliente=p.codigo_cliente GROUP BY
c.codigo_cliente;
...
SELECT c.nombre_cliente AS Nombres, SUM(p.total) As Total
FROM cliente c, pago p WHERE c.codigo_cliente=p.codigo_cliente GROUP BY
c.codigo_cliente;
/*3. Devuelve el nombre de los clientes que hayan hecho pedidos en 2008
ordenados alfabéticamente de menor a mayor.*/
select c.nombre_cliente,p.fecha_pedido from cliente c
inner join pedido p
on p.codigo_cliente=c.codigo_cliente
where year(p.fecha_pedido)='2008'
group by c.nombre_cliente;

sELECT c.nombre_cliente AS Nombre, p.fecha_pedido As Fecha_del_Pedido
FROM cliente c INNER JOIN pedido p ON c.codigo_cliente=p.codigo_cliente
WHERE p.fecha_pedido >= '2008' AND p.fecha_pedido <='2008-12-31'
GROUP BY nombre_cliente ASC;
/*4. Devuelve el nombre del cliente, el nombre y primer apellido de su
representante de ventas y el número de teléfono de la oficina del
representante de ventas, de aquellos clientes que no hayan realizado
ningún pago.*/

select c.nombre_cliente,e.nombre,e.apellido1,e.apellido2,o.telefono from cliente c
inner join empleado e
on e.codigo_empleado=c.codigo_empleado_rep_ventas
inner join oficina o
on e.codigo_oficina=o.codigo_oficina
...
SELECT c.nombre_cliente AS Nombre_del_Cliente, e.nombre Nombre_Empleado,e.apellido1 As Primer_Apellido, e.apellido2 As Segundo_Apellido, o.telefono As Telefono FROM cliente c 
INNER JOIN empleado e 
ON c.codigo_empleado_rep_ventas=e.codigo_empleado 
INNER JOIN oficina o 
ON o.codigo_oficina=e.codigo_oficina 
WHERE NOT EXISTS (SELECT pago.codigo_cliente FROM pago WHERE codigo_cliente=c.codigo_cliente);
...
SELECT c.nombre_cliente AS Nombre_del_Cliente, e.nombre Nombre_Empleado,
e.apellido1 As Primer_Apellido, e.apellido2 As Segundo_Apellido, o.ciudad AS Ciudad
FROM cliente c INNER JOIN empleado e ON
c.codigo_empleado_rep_ventas=e.codigo_empleado INNER JOIN oficina o ON
o.codigo_oficina=e.codigo_oficina WHERE EXISTS (SELECT oficina.codigo_oficina
FROM oficina WHERE codigo_oficina=e.codigo_oficina);
...
where not exists(select codigo_empleado_rep_ventas from cliente where codigo_empleado_rep_ventas=e.codigo_empleado);


SELECT e.nombre , e.apellido1 , e.apellido2, e.puesto , o.telefono AS Telefono FROM empleado e 
INNER JOIN oficina o 
ON o.codigo_oficina=e.codigo_oficina 
INNER JOIN cliente c 
ON e.codigo_empleado=c.codigo_cliente 
...
INNER JOIN empleado e 
ON o.codigo_oficina=e.codigo_oficina 
WHERE EXISTS (SELECT codigo_oficina FROM empleado WHERE codigo_oficina=o.codigo_oficina) 
GROUP BY ciudad;
/**/
/**/
/*3. Devuelve el nombre del producto del que se han vendido más unidades.
...
SELECT producto.nombre, detalle_pedido.cantidad FROM producto 
INNER JOIN detalle_pedido 
ON producto.codigo_producto=detalle_pedido.codigo_producto 
WHERE cantidad =(SELECT MAX(cantidad) from detalle_pedido);

select p.nombre, sum(dp.cantidad) from producto p
inner join detalle_pedido dp on dp.codigo_producto = p.codigo_producto
group by p.nombre
having sum(dp.cantidad)=(SELECT sum(max(d.cantidad)) from detalle_pedido d where d.codigo_producto);
*/
